...
void* muti(void* args) {
    ...
    while (1) {
        ...
        /*	strcpy(buffer,catCharInt(buffer,clientId));
                for (list<int>::iterator iter = client.begin(); iter !=
           client.end(); ++iter) { if (*iter != clientId) { send(*iter, buffer,
           strlen(buffer) + 1, 0);
                        }
                }*/
    }
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        if (client_socket < 0) {
            ...
            ret =
                pthread_create(&id, NULL, muti, &client_socket);  // 创建子进程
            /* snprintf(c, sizeof(c), "%d", client.size());
             strcpy(buffer,c);
             for (list<int>::iterator iter = client.begin(); iter !=
             client.end(); ++iter) { send(*iter, buffer, strlen(buffer) + 1, 0);
             //更新用户总数
                         }*/
        }
